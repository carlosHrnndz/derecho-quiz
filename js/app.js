...
class QuizApp {
    ...
    getFirebaseRef() {
        ...
        return firebase.database().ref(`sessions_derecho/${this.sessionCode}`);
    }
    ...
    loadStoredSessionCode() {
        const stored = localStorage.getItem('quizSessionCode_derecho');
        if (stored) {
    ...
    saveSessionCode() {
        ...
        if (this.sessionCode) localStorage.setItem('quizSessionCode_derecho', this.sessionCode);
    }

    getStorageKey() {
        return this.sessionCode ? `derecho_progress_${this.sessionCode}` : 'derecho_progress_default';
    }
    ...
    async openLeaderboard() {
        ...
        try {
            const ref = firebase.database().ref('leaderboard_derecho');
    ...
    async submitScore() {
        ...
        try {
            ...
            await firebase.database().ref('leaderboard_derecho').push(data);
